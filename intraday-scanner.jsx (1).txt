...
const STOCK_LIST = [
  ...
  { symbol: "M%26M.NS", name: "M&M", sector: "Auto" },
  { symbol: "ULTRACEMCO.NS", name: "UltraTech Cem.", sector: "Cement" },
  ...
  { symbol: "IOC.NS", name: "IOC", sector: "Energy" },
  ...
  { symbol: "LICHSGFIN.NS", name: "LIC Housing", sector: "Finance" },
  ...
  // Auto Ancillary
  { symbol: "BOSCHLTD.NS", name: "Bosch", sector: "Auto Anc." },
  ...
  { symbol: "NYKAA.NS", name: "Nykaa", sector: "Tech" },
  ...
  { symbol: "AMBER.NS", name: "Amber Ent.", sector: "Electronics" },
  { symbol: "KAYNES.NS", name: "Kaynes Tech", sector: "Electronics" },
  // Pharma / Healthcare
  { symbol: "LUPIN.NS", name: "Lupin", sector: "Pharma" },
  { symbol: "AUROPHARMA.NS", name: "Aurobindo", sector: "Pharma" },
  ...
  { symbol: "FORTIS.NS", name: "Fortis Health", sector: "Healthcare" },
  ...
  { symbol: "JUBLFOOD.NS", name: "Jubilant Foods", sector: "FMCG" },
  { symbol: "DEVYANI.NS", name: "Devyani Intl.", sector: "FMCG" },
  { symbol: "WESTLIFE.NS", name: "Westlife Dev.", sector: "FMCG" },
  // Telecom / Media
  ...
  { symbol: "SUNTV.NS", name: "Sun TV", sector: "Media" },
  ...
  { symbol: "DOMS.NS", name: "DOMS Ind.", sector: "Consumer" },
];
...
function calcRSI(closes, period = 14) {
  ...
  for (let i = period+1; i < closes.length; i++) { const d=closes[i]-closes[i-1]; ag=(ag*(period-1)+Math.max(0,d))/period; al=(al*(period-1)+Math.max(0,-d))/period; }
  if (al===0) return 100;
...
function analyzeStock(candles) {
  ...
  const ema9arr=calcEMA(closes,9), ema21arr=calcEMA(closes,21);
  const pattern=detectPattern(candles.slice(-3));
  const last=closes[closes.length-1];
  const ema9=ema9arr[ema9arr.length-1], ema21=ema21arr[ema21arr.length-1];
  ...
  if (ema9&&ema21) { if(ema9>ema21){score+=15;reasons.push("EMA9>EMA21 ↑");}else{score-=12;reasons.push("EMA9<EMA21 ↓");} }
  if (last>vwap){score+=10;reasons.push("Above VWAP");}else{score-=8;reasons.push("Below VWAP");}
  ...
  return {
    ...
    ema9:ema9?.toFixed(2),ema21:ema21?.toFixed(2),vwap:vwap.toFixed(2),atr:atr.toFixed(2),
  ...
  return(
    <svg width={w} height={h} style={{display:"block",overflow:"visible"}}>
      {[minP,(minP+maxP)/2,maxP].map((p,i)=>{
        const y=toY(p);
        return(<g key={i}><line x1={pl.l} y1={y} x2={w-pl.r} y2={y} stroke="#ffffff08" strokeWidth="1"/><text x={pl.l-3} y={y+3.5} fill="#3a5060" fontSize="7" textAnchor="end">{fmt(p)}</text></g>);
      })}
      {candles.map((c,i)=>{
        const bull=c.close>=c.open, col=bull?"#00d97e":"#ff4d4d";
        const bT=toY(Math.max(c.open,c.close)), bB=toY(Math.min(c.open,c.close));
        const bH=Math.max(1,bB-bT), x=toX(i);
        return(<g key={i}><line x1={x} y1={toY(c.high)} x2={x} y2={toY(c.low)} stroke={col} strokeWidth={1} opacity={0.8}/><rect x={x-cw/2} y={bT} width={cw} height={bH} fill={col} opacity={0.9} rx="0.5"/></g>);
      })}
    </svg>
  );
}
...
function Loader({progress,done,total,current}){
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"70vh",gap:"16px"}}>
      ...
      <div style={{color:"#00ff88",fontSize:"13px",fontWeight:"700",letterSpacing:"4px"}}>SCANNING {total} NSE STOCKS</div>
      <div style={{width:"280px",height:"5px",background:"#0d1520",borderRadius:"3px",overflow:"hidden"}}>
      ...
      <div style={{color:"#ffd700",fontSize:"11px"}}>{done} / {total} · {progress}%</div>
      <div style={{color:"#2d4a60",fontSize:"9px",letterSpacing:"0.5px",maxWidth:"300px",textAlign:"center"}}>{current}</div>
      <div style={{color:"#161e2a",fontSize:"8px"}}>5-Min Candles · RSI · EMA9/21 · VWAP · ATR · Volume · Pattern</div>
    </div>
  );
}
...
export default function IntradayScanner(){
  ...
  const fetchOne=async(symbol)=>{
    try{
      ...
      const candles=ts.map((t,i)=>({time:new Date(t*1000),open:q.open?.[i],high:q.high?.[i],low:q.low?.[i],close:q.close?.[i],volume:q.volume?.[i]||0}))
  ...
  const loadAll=useCallback(async()=>{
    setLoading(true);setProg(0);setDone(0);setErr(null);
    const nd={};let d=0;
    const B=4;
    for(let i=0;i<STOCK_LIST.length;i+=B){
      const batch=STOCK_LIST.slice(i,i+B);
      setCurrent(batch.map(s=>s.name).join(" · "));
      const res=await Promise.all(batch.map(s=>fetchOne(s.symbol)));
      res.forEach((candles,j)=>{
        const st=batch[j];
        if(candles&&candles.length>=22){const a=analyzeStock(candles);if(a)nd[st.symbol]={...st,...a};}
        d++;
      });
      setDone(d);setProg(Math.round(d/STOCK_LIST.length*100));
      await new Promise(r=>setTimeout(r,110));
    }
    if(Object.keys(nd).length===0) setErr("No data received. NSE Market hours: 9:15 AM – 3:30 PM IST (Mon–Fri). Yahoo Finance may also be temporarily unavailable.");
    setData(nd);setLastSync(new Date());setLoading(false);setPage(0);
  },[]);

  useEffect(()=>{loadAll();const iv=setInterval(loadAll,5*60*1000);return()=>clearInterval(iv);},[loadAll]);

  const istTime=()=>{
    const ist=new Date(time.toLocaleString("en-US",{timeZone:"Asia/Kolkata"}));
    const m=ist.getHours()*60+ist.getMinutes();
    return{m,ist,open:m>=555&&m<=930};
  };
  const {open:marketOpen,ist}=istTime();

  const fmtT=d=>d?.toLocaleString("en-IN",{timeZone:"Asia/Kolkata",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:true})??"---";

  const stocks=Object.values(data);
  const filtered=stocks.filter(s=>{
    if(sig==="BUY"&&!s.signal.includes("BUY")) return false;
    if(sig==="SELL"&&!s.signal.includes("SELL")) return false;
    if(sig==="NEUTRAL"&&s.signal!=="NEUTRAL") return false;
    ...
    if(sort==="price↑") return parseFloat(a.lastClose)-parseFloat(b.lastClose);
    if(sort==="price↓") return parseFloat(b.lastClose)-parseFloat(a.lastClose);
    return 0;
  });

  const topBuys=stocks.filter(s=>s.signal==="STRONG BUY").sort((a,b)=>b.score-a.score).slice(0,6);
  const pages=Math.ceil(filtered.length/PAGE_SIZE);
  const paged=filtered.slice(page*PAGE_SIZE,(page+1)*PAGE_SIZE);

  const inp=(label,value,setter,opts={})=>(
    <div>
      <div style={{color:"#2d4060",fontSize:"7px",letterSpacing:"0.8px",marginBottom:"3px"}}>{label}</div>
      <input type="number" value={value} onChange={e=>setter(+e.target.value)} {...opts}
        style={{background:"#080d18",border:"1px solid #0e1e2e",color:"#ffd700",padding:"5px 8px",borderRadius:"4px",fontSize:"11px",width:opts.width||"90px",outline:"none",fontFamily:"inherit"}}/>
    </div>
  );

  const Chip=({label,active,onClick,ac="#00ff88"})=>(
    <button onClick={onClick} style={{background:active?`${ac}12`:"transparent",border:`1px solid ${active?ac+"35":"#0d1828"}`,color:active?ac:"#2d4060",padding:"3px 8px",borderRadius:"10px",cursor:"pointer",fontSize:"7.5px",fontFamily:"inherit",whiteSpace:"nowrap"}}>{label}</button>
  );

  const Stat=(l,v,c)=>(
    <div style={{background:"#060c16",borderRadius:"5px",padding:"6px 8px",textAlign:"center"}}>
  ...
  return(
    <div style={{fontFamily:"'JetBrains Mono','Courier New',monospace",background:"#03060d",color:"#a8c0d0",minHeight:"100vh"}}>
      {/* Header */}
      <div style={{background:"#050910",borderBottom:"1px solid #00ff8815",padding:"8px 14px",position:"sticky",top:0,zIndex:200,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"6px",boxShadow:"0 2px 20px #00000099"}}>
        <div>
          <span style={{color:"#00ff88",fontSize:"14px",fontWeight:"700",letterSpacing:"3px"}}>◈ NSE/BSE INTRADAY SCANNER</span>
          <span style={{color:"#152030",fontSize:"8px",marginLeft:"8px"}}>· {STOCK_LIST.length} STOCKS · LIVE 5-MIN</span>
        </div>
        <div style={{display:"flex",gap:"10px",alignItems:"center"}}>
          <div style={{color:marketOpen?"#00ff88":"#ff5555",fontSize:"8px",padding:"3px 10px",borderRadius:"10px",background:marketOpen?"#00ff8810":"#ff555510",border:`1px solid ${marketOpen?"#00ff8825":"#ff555525"}`,fontWeight:"700"}}>
            ● {marketOpen?"MARKET OPEN":"CLOSED"}
          </div>
          <div style={{color:"#3a5068",fontSize:"9px"}}>{fmtT(time)} IST</div>
          {lastSync&&<div style={{color:"#1a2a38",fontSize:"8px"}}>↻ {fmtT(lastSync)}</div>}
          <button onClick={loadAll} style={{background:"#0a1420",border:"1px solid #00ff8825",color:"#00ff88",padding:"4px 12px",borderRadius:"4px",cursor:"pointer",fontSize:"9px",letterSpacing:"1px",fontFamily:"inherit"}}>↻ SCAN</button>
        </div>
      </div>

      {loading&&<Loader progress={prog} done={done} total={STOCK_LIST.length} current={current}/>}

      {!loading&&err&&(
        <div style={{padding:"50px 20px",textAlign:"center"}}>
          <div style={{fontSize:"30px",marginBottom:"10px"}}>📴</div>
          <div style={{color:"#ff6040",fontSize:"12px",marginBottom:"8px"}}>⚠ {err}</div>
          <button onClick={loadAll} style={{background:"transparent",border:"1px solid #00ff8835",color:"#00ff88",padding:"8px 20px",borderRadius:"6px",cursor:"pointer",fontSize:"10px",letterSpacing:"2px",fontFamily:"inherit",marginTop:"12px"}}>↻ RETRY</button>
        </div>
      )}
      ...
      {!loading&&!err&&(
        <div style={{padding:"10px 12px"}}>
          ...
          {/* Settings */}
          <div style={{background:"#060b16",border:"1px solid #0a1828",borderRadius:"8px",padding:"10px 12px",marginBottom:"10px",display:"flex",gap:"10px",flexWrap:"wrap",alignItems:"flex-end"}}>
            {inp("CAPITAL (₹)",capital,setCapital,{width:"100px"})}
            {inp("MIN INVEST (₹)",minInv,setMinInv,{min:100,width:"75px"})}
            {inp("RISK %",riskPct,setRiskPct,{min:0.25,max:5,step:0.25,width:"60px"})}
            <div>
              <div style={{color:"#2d4060",fontSize:"7px",marginBottom:"3px"}}>SEARCH</div>
              <input type="text" value={search} onChange={e=>{setSearch(e.target.value);setPage(0);}} placeholder="Name / Symbol..." style={{background:"#080d18",border:"1px solid #0e1e2e",color:"#a8c0d0",padding:"5px 8px",borderRadius:"4px",fontSize:"11px",width:"120px",outline:"none",fontFamily:"inherit"}}/>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:"5px",cursor:"pointer",paddingBottom:"2px"}} onClick={()=>{setAffordable(v=>!v);setPage(0);}}>
              <div style={{width:"14px",height:"14px",borderRadius:"3px",background:affordable?"#00ff88":"transparent",border:"1px solid #00ff8835",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                {affordable&&<span style={{color:"#000",fontSize:"9px",fontWeight:"900",lineHeight:1}}>✓</span>}
              </div>
              <span style={{color:"#3a5060",fontSize:"8px"}}>≤ ₹{minInv}/share</span>
            </div>
          </div>

          {/* Signal Filter */}
          <div style={{display:"flex",gap:"5px",flexWrap:"wrap",marginBottom:"8px",alignItems:"center"}}>
            <span style={{color:"#1e3040",fontSize:"7px",letterSpacing:"1px",marginRight:"2px"}}>SIGNAL:</span>
            {["ALL","BUY","SELL","NEUTRAL"].map(f=><Chip key={f} label={f} active={sig===f} onClick={()=>{setSig(f);setPage(0);}} ac="#00ff88"/>)}
            <span style={{color:"#1e3040",fontSize:"7px",letterSpacing:"1px",margin:"0 4px"}}>SORT:</span>
            {["score","change","rsi","price↑","price↓"].map(s=><Chip key={s} label={s.toUpperCase()} active={sort===s} onClick={()=>{setSort(s);setPage(0);}} ac="#00bbff"/>)}
          </div>

          {/* Sector Filter */}
          <div style={{display:"flex",gap:"4px",flexWrap:"wrap",marginBottom:"12px"}}>
            {SECTORS.map(s=><Chip key={s} label={s} active={sector===s} onClick={()=>{setSector(s);setPage(0);}} ac="#cc88ff"/>)}
          </div>

          {/* Stat Summary */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:"6px",marginBottom:"14px"}}>
            {[["Analyzed",stocks.length,"#a0c0d0"],["Strong Buy",stocks.filter(s=>s.signal==="STRONG BUY").length,"#00ff88"],["Buy",stocks.filter(s=>s.signal==="BUY").length,"#7fff00"],["Neutral",stocks.filter(s=>s.signal==="NEUTRAL").length,"#ffd700"],["Sell/Avoid",stocks.filter(s=>s.signal.includes("SELL")).length,"#ff5555"]].map(([l,v,c])=>(
              <div key={l} style={{background:"#050a14",border:"1px solid #0a1520",borderRadius:"6px",padding:"8px",textAlign:"center"}}>
          ...
          {topBuys.length>0&&(
            <div style={{marginBottom:"16px"}}>
              <div style={{color:"#2d4060",fontSize:"8px",letterSpacing:"2px",marginBottom:"8px"}}>
                <span style={{color:"#00ff88"}}>▶</span> TODAY'S STRONGEST BUY SIGNALS — Click for full trade plan
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"8px"}}>
                {topBuys.map(s=>{
                  ...
                  return(
                    <div key={s.symbol} onClick={()=>setSelected(s)} style={{background:"linear-gradient(145deg,#040e09,#050a14)",border:`1px solid ${s.color}20`,borderRadius:"10px",padding:"10px",cursor:"pointer",position:"relative",overflow:"hidden"}}>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:"4px"}}>
                        <div>
                          <div style={{color:"#ddeeff",fontSize:"11px",fontWeight:"700"}}>{s.name}</div>
                          <div style={{color:"#2d4060",fontSize:"7px"}}>{s.sector} · ₹{s.lastClose}</div>
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{color:s.color,fontSize:"7px",fontWeight:"700",padding:"1px 5px",borderRadius:"2px",background:`${s.color}18`}}>{s.signal}</div>
                          <div style={{color:"#ffd700",fontSize:"9px",marginTop:"2px"}}>⚡{s.score}</div>
                        </div>
                      </div>
                      <MiniChart candles={s.candles} w={180} h={70}/>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"3px",marginTop:"6px"}}>
                        {[["ENTRY",`₹${s.entry}`,"#00bbff"],["TARGET",`₹${s.target}`,"#00ff88"],["SL",`₹${s.stopLoss}`,"#ff4444"]].map(([l,v,c])=>(
                          <div key={l} style={{background:"#040810",borderRadius:"4px",padding:"4px 5px",textAlign:"center"}}>
                            ...
                            <div style={{color:c,fontSize:"8px",fontWeight:"700"}}>{v}</div>
                          </div>
                        ))}
                      </div>
          ...
          {/* Full Table */}
          <div>
            <div style={{color:"#2d4060",fontSize:"8px",letterSpacing:"1px",marginBottom:"6px"}}>
              <span style={{color:"#00bbff"}}>▶</span> {filtered.length} STOCKS MATCHED · Page {page+1}/{Math.max(1,pages)}
            </div>
            <div style={{background:"#050910",border:"1px solid #0a1520",borderRadius:"8px",overflow:"hidden"}}>
              <div style={{display:"grid",gridTemplateColumns:"1.3fr 0.7fr 65px 55px 40px 110px 60px 90px",padding:"6px 10px",borderBottom:"1px solid #090f1c",fontSize:"6.5px",color:"#1e3040",letterSpacing:"1px"}}>
                ...
                <div>PATTERN</div><div style={{textAlign:"right"}}>SCORE</div><div style={{textAlign:"right"}}>ENTRY / SL / TGT</div>
              </div>
              {paged.map((s,i)=>{
                const qty=calcQty(capital,s.entry,s.stopLoss,riskPct,minInv);
                const inv=(qty*parseFloat(s.entry)).toFixed(0);
                return(
                  <div key={s.symbol} onClick={()=>setSelected(s)} style={{display:"grid",gridTemplateColumns:"1.3fr 0.7fr 65px 55px 40px 110px 60px 90px",padding:"7px 10px",borderBottom:"1px solid #07090f",cursor:"pointer",background:i%2?"#06090f50":"transparent",fontSize:"10px",alignItems:"center"}}>
                    <div>
                      <div style={{color:"#d0e4f0",fontWeight:"600",fontSize:"10px"}}>{s.name}</div>
                      <div style={{color:"#182430",fontSize:"7px"}}>{s.sector}</div>
                    </div>
                    <div><span style={{color:SIG_COL[s.signal]||"#ffd700",fontSize:"7px",fontWeight:"700",padding:"1px 4px",borderRadius:"2px",background:`${SIG_COL[s.signal]||"#ffd700"}12`}}>{s.signal}</span></div>
                    ...
                    <div style={{textAlign:"right"}}>
                      <div style={{display:"flex",alignItems:"center",gap:"3px",justifyContent:"flex-end"}}>
                        <div style={{width:"28px",height:"3px",background:"#080e18",borderRadius:"2px"}}>
                    ...
                    <div style={{textAlign:"right",fontSize:"7.5px",lineHeight:"1.6"}}>
                      <span style={{color:"#00bbff"}}>₹{s.entry}</span> <span style={{color:"#1a2838"}}>/</span> <span style={{color:"#ff4444"}}>₹{s.stopLoss}</span>
              ...
              {paged.length===0&&<div style={{padding:"30px",textAlign:"center",color:"#2d4060",fontSize:"11px"}}>No stocks match your filters</div>}
            </div>

            {/* Pagination */}
            {pages>1&&(
              <div style={{display:"flex",justifyContent:"center",gap:"5px",marginTop:"10px",flexWrap:"wrap"}}>
                <button onClick={()=>setPage(p=>Math.max(0,p-1))} disabled={page===0} style={{background:"transparent",border:"1px solid #0d1828",color:page===0?"#152030":"#3a5060",padding:"4px 10px",borderRadius:"4px",cursor:page===0?"default":"pointer",fontSize:"9px",fontFamily:"inherit"}}>← Prev</button>
                {Array.from({length:pages},(_,i)=>(
                  <button key={i} onClick={()=>setPage(i)} style={{background:page===i?"#0a1828":"transparent",border:`1px solid ${page===i?"#00bbff35":"#0d1828"}`,color:page===i?"#00bbff":"#2d4060",padding:"4px 8px",borderRadius:"4px",cursor:"pointer",fontSize:"9px",fontFamily:"inherit"}}>{i+1}</button>
                ))}
                <button onClick={()=>setPage(p=>Math.min(pages-1,p+1))} disabled={page===pages-1} style={{background:"transparent",border:"1px solid #0d1828",color:page===pages-1?"#152030":"#3a5060",padding:"4px 10px",borderRadius:"4px",cursor:page===pages-1?"default":"pointer",fontSize:"9px",fontFamily:"inherit"}}>Next →</button>
              </div>
            )}
          </div>

          <div style={{marginTop:"14px",padding:"8px 12px",background:"#040710",borderRadius:"6px",color:"#182838",fontSize:"7.5px",lineHeight:"1.7"}}>
            ⚠ DISCLAIMER: Educational & informational purpose only. Not SEBI-registered financial advice. Intraday trading is highly risky — you may lose your entire investment. Always use stop-loss. Never invest borrowed money. Past patterns ≠ future results. Data: Yahoo Finance 5-min NSE candles. Auto-refreshes every 5 minutes.
          </div>
        </div>
      )}
      ...
      {selected&&(()=>{
        ...
        return(
          <div style={{position:"fixed",inset:0,background:"#000000d8",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:"10px"}} onClick={()=>setSelected(null)}>
            <div style={{background:"#060a14",border:`1px solid ${selected.color}25`,borderRadius:"12px",padding:"16px",maxWidth:"540px",width:"100%",maxHeight:"94vh",overflowY:"auto",position:"relative",boxShadow:`0 0 80px ${selected.color}0a`}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                <div>
                  <div style={{color:"#ffffff",fontSize:"17px",fontWeight:"700"}}>{selected.name}</div>
                  <div style={{color:"#2d4060",fontSize:"8px"}}>{selected.symbol} · NSE · {selected.sector}</div>
                </div>
                <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                  <span style={{color:selected.color,fontSize:"8px",fontWeight:"700",padding:"3px 10px",borderRadius:"4px",background:`${selected.color}18`}}>{selected.signal}</span>
                  <button onClick={()=>setSelected(null)} style={{background:"transparent",border:"none",color:"#3a5060",fontSize:"18px",cursor:"pointer",lineHeight:1}}>✕</button>
                </div>
              </div>

              <div style={{background:"#02050d",borderRadius:"6px",padding:"8px",marginBottom:"12px"}}>
                <MiniChart candles={selected.candles} w={508} h={160}/>
              </div>

              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"6px",marginBottom:"12px"}}>
                {[
                  ["LTP",`₹${selected.lastClose}`,"#fff"],
                  ["CHANGE",`${parseFloat(selected.change)>=0?"+":""}${selected.change}%`,parseFloat(selected.change)>=0?"#00d97e":"#ff5555"],
                  ["SCORE",`${selected.score}/100`,"#ffd700"],
                  ["RSI (14)",selected.rsi??"—",selected.rsi<30?"#00ff88":selected.rsi>70?"#ff5555":"#7a9ab0"],
                  ["EMA 9",selected.ema9,"#ffd700"],
                  ["EMA 21",selected.ema21,"#ff8800"],
                  ["VWAP",`₹${selected.vwap}`,"#00bbff"],
                  ["ATR",selected.atr,"#cc88ff"],
                  ["D-HIGH",`₹${selected.high}`,"#00ff88"],
                  ["D-LOW",`₹${selected.low}`,"#ff5555"],
                  ["PATTERN",selected.pattern.split(" ")[0],"#7a9ab0"],
                  ["SECTOR",selected.sector,"#5a7080"],
                ].map(([l,v,c])=>(
              ...
              {/* Trade Plan */}
              <div style={{background:isBuy?"#030e08":"#0d0306",border:`1px solid ${selected.color}15`,borderRadius:"8px",padding:"14px",marginBottom:"12px"}}>
                <div style={{color:selected.color,fontSize:"8px",letterSpacing:"2px",marginBottom:"10px"}}>▶ INTRADAY TRADE PLAN — {isBuy?"LONG (BUY)":"SHORT / AVOID"}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"7px"}}>
                  {[
                    ["BUY PRICE (Entry)",`₹${selected.entry}`,"#00bbff"],
                    ["TARGET",`₹${selected.target}  (+${pctT}%)`,"#00ff88"],
                    ["STOP LOSS",`₹${selected.stopLoss}  (${pctSL}%)`,"#ff4444"],
                    ["RISK : REWARD",`1 : ${rr}`,"#ffd700"],
                    ["QTY TO BUY",`${qty} shares`,"#ffd700"],
                    ["TOTAL INVESTMENT",`₹${parseInt(inv).toLocaleString("en-IN")}`,"#c8d8e0"],
                    ["MAX PROFIT",`₹${parseInt(profit).toLocaleString("en-IN")}`,"#00ff88"],
                    ["MAX LOSS",`₹${parseInt(maxLoss).toLocaleString("en-IN")}`,"#ff5555"],
                  ].map(([l,v,c])=>(
                    <div key={l} style={{background:"#060c16",borderRadius:"5px",padding:"8px 10px"}}>
                      <div style={{color:"#2d4060",fontSize:"7px",marginBottom:"2px"}}>{l}</div>
                      <div style={{color:c,fontSize:"13px",fontWeight:"700"}}>{v}</div>
                    </div>
                  ))}
                </div>
                <div style={{marginTop:"10px",padding:"10px",background:"#060c16",borderRadius:"6px",color:"#3a5060",fontSize:"8.5px",lineHeight:"1.7"}}>
                  {isBuy
                    ?`💡 Enter BUY at ₹${selected.entry}. Set stop-loss at ₹${selected.stopLoss} (${Math.abs(parseFloat(pctSL)).toFixed(1)}% below entry). Target ₹${selected.target} (+${pctT}%). Buy ${qty} shares = ₹${parseInt(inv).toLocaleString("en-IN")} total investment. ⚠ Compulsory exit before 3:20 PM IST. Max risk: ₹${parseInt(maxLoss).toLocaleString("en-IN")}.`
                    :`⚠ Signal is BEARISH. Avoid buying this stock today. Wait for reversal signal or skip. If short-selling, entry ₹${selected.entry}, SL ₹${selected.stopLoss}, target ₹${selected.target}.`}
                </div>
              </div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:"4px"}}>
                {[...selected.reasons,selected.pattern].map((r,i)=>(
                  <span key={i} style={{background:"#07101a",border:"1px solid #0d1a28",color:"#3a5870",fontSize:"8px",padding:"3px 8px",borderRadius:"10px"}}>{r}</span>
                ))}
              </div>
            </div>
          </div>
